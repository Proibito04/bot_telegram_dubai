...
import {impostaCookie} from "./scrapa";
...
export const echoBot = functions.region('europe-west1').runWith({
    memory: '1GB',
    timeoutSeconds: 120
...
export const salvaCookie = functions.region('europe-west1').runWith({
    memory: '1GB',
    timeoutSeconds: 120
}).https.onRequest(async (request: any, response) => {
    await impostaCookie()
